...
import { carsAPI, propertiesAPI, purchaseRequestsAPI } from '../../services/api'
...
const AdminDashboard = () => {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: async () => {
      const [carsRes, propertiesRes, requestsRes] = await Promise.all([
        carsAPI.getAll({ limit: 1 }),
        propertiesAPI.getAll({ limit: 1 }),
        purchaseRequestsAPI.getAll({ limit: 1 })
      ])
      return {
  ...
  const { data: recentRequests, isLoading: requestsLoading } = useQuery({
    ...
    queryFn: () => purchaseRequestsAPI.getAll({ limit: 10, sort: 'newest' }).then(res => res.data.requests || [])
  })
