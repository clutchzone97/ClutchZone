...
const AddProperty = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || t('Error adding property');
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white border-b border-gray-200 rounded-t-lg p-6">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{t('Add New Property')}</h1>
              <p className="text-gray-600 mt-1">{t('Enter details for the new property')}</p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <input type="text" name="location" value={formData.location} onChange={handleChange} required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" />
              </div>

              {/* Bedrooms */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Bedrooms')}</label>
                <input type="number" name="bedrooms" value={formData.bedrooms} onChange={handleChange} required min="0" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" />
              </div>

              {/* Bathrooms */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Bathrooms')}</label>
                <input type="number" name="bathrooms" value={formData.bathrooms} onChange={handleChange} required min="0" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" />
              </div>

              {/* Area */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Area (sqm)')}</label>
                <input type="number" name="area" value={formData.area} onChange={handleChange} required min="0" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" />
              </div>
            ...
            <div className="flex justify-end">
              <button type="submit" disabled={isSubmitting} className={`px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}>
                {isSubmitting ? t('Adding...') : t('Add Property')}
              </button>
            </div>
          </form>
        </div>
